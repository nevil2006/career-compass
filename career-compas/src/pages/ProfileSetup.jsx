...
const ProfileSetup = () => {
  ...
  const handleSubmit = (e) => {
    ...
    // ✅ Save data in localStorage (simulate database for now)
    localStorage.setItem("userProfile", JSON.stringify(formData));

    alert("🎉 Profile setup complete! Redirecting to your AI career suggestions...");
    
    // ✅ Redirect to CareerRecommendation page
    navigate("/careerrecommend");
  };
              ...
              <div className="tag-list">
                {skills.map((s) => (
                  <button
                    key={s}
                    type="button"
                    className={formData.skills.includes(s) ? "tag selected" : "tag"}
              ...
              <div className="tag-list">
                {frameworks.map((f) => (
                  <button
                    key={f}
                    type="button"
                    className={formData.frameworks.includes(f) ? "tag selected" : "tag"}
                    onClick={() => handleMultiSelect("frameworks", f)}
                  >
                    {f}
                  </button>
                ))}
              ...
              <div className="tag-list">
                {databases.map((d) => (
                  <button
                    key={d}
                    type="button"
                    className={formData.databases.includes(d) ? "tag selected" : "tag"}
                    onClick={() => handleMultiSelect("databases", d)}
                  >
                    {d}
                  </button>
                ))}
              ...
              <div className="tag-list">
                {domains.map((domain) => (
                  <button
                    key={domain}
                    type="button"
                    className={formData.domain.includes(domain) ? "tag selected" : "tag"}
                    onClick={() => handleMultiSelect("domain", domain)}
                  >
                    {domain}
                  </button>
                ))}
          ...
          {step === 5 && (
            ...
            >
              ...
              <div className="ai-summary">
                ...
                <p>
                  “Based on your interests, you may fit well in <b>AI Engineer</b> or <b>Data Scientist</b> roles.”
                </p>
              </div>
            </motion.div>
          )}
